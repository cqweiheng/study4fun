...
public class ProductController {
	...
	public ResponseEntity<Product> hystrixproduct(@PathVariable Long id){
		Product product = productService.getProductTreadIsolution(id);
		return ResponseEntity.ok(product);
	}

	@GetMapping("/hystrixproduct2/{id}")
	public ResponseEntity<Product> hystrixproduct2(@PathVariable Long id){
		Product product = productService.getProductTreadIsolution2(id);
		return ResponseEntity.ok(product);
	}
